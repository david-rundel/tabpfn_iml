...
from tabpfniml.tabpfn_interpret.utils import get_uniform_single_eval_pos_sampler
import torch
import math
...
def get_mlp_prior_hyperparameters(config):
    from tabpfn_interpret.priors.utils import gamma_sampler_f
...
def get_meta_gp_prior_hyperparameters(config):
    from tabpfn_interpret.priors.utils import trunc_norm_sampler_f
    config = {hp: (list(config[hp].values())[0]) if type(config[hp]) is dict else config[hp] for hp in config}
...
def get_model(config, device, should_train=True, verbose=False, state_dict=None, epoch_callback=None):
    import tabpfn_interpret.priors as priors
    from tabpfn_interpret.train import train, Losses
    extra_kwargs = {}
