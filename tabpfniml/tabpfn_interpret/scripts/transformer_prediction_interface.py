...
from tabpfniml.tabpfn_interpret.utils import normalize_data, to_ranking_low_mem, remove_outliers
from tabpfniml.tabpfn_interpret.utils import NOP, normalize_by_used_features_f
...
from tabpfniml.tabpfn_interpret.scripts.model_builder import load_model, load_model_only_inference
import os
import pickle
import io
...
class TabPFNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, device='cpu', base_path=pathlib.Path(__file__).parent.parent.resolve(), model_string='',
                 ...
                 store_gradients= False, classes_= None): #Edit Rundel: Add store_gradients
        ...
        if isinstance(classes_, np.ndarray):
    ...
    def fit(self, X, y, overwrite_warning=False):
        ...
        #Edit Rundel: As in https://github.com/automl/TabPFN/pull/30/files
        #Comment out when storing gradients, since we cannot map tensor to numpy and maintain gradient
    ...
    def predict_proba(self, X, normalize_with_test=False):
        ...
        if not self.store_gradients:
            ...
            #Use torch-alternative to np.concatenate
            X_full= torch.cat([self.X_, X], dim=0).float().unsqueeze(1).to(self.device)
            y_full= torch.cat([self.y_, torch.zeros(X.shape[0])], dim=0).float().unsqueeze(1).to(self.device)
...
def transformer_predict(model, eval_xs, eval_ys, eval_position,
    ...
    def predict(eval_xs, eval_ys, used_style, softmax_temperature, return_logits):
        ...
        inference_mode_call = torch.inference_mode() if inference_mode and not store_gradients else NOP() #Rundel:NOP leads to default/grad-mode?
    ...
    def preprocess_input(eval_xs, preprocess_transform):
        ...
        #Edit Rundel: Do not detach  in order for gradient flow
...
#                return 'robust_all'
    ...
    for ensemble_configuration in ensemble_configurations:
        ...
        eval_xs_, eval_ys_ = eval_xs.clone(), eval_ys.clone() #Comment Rundel: Clone does not remove from comp. graph
        ...
        if preprocess_transform_configuration in eval_xs_transformed:
            ...
            if eval_xs_.shape[-1] * 3 < max_features and combine_preprocessing and not store_gradients:
                ...
                #Edit Rundel: Until autograd-Method implemented
                if store_gradients:
                    preprocess_transform_configuration= "none"
