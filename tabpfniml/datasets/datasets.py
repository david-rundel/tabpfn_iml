...
class dataset_iml(ABC):
    ...
    def __init__(self):
        ...
        assert hasattr(
            self, "id"), "The 'id' attribute has to be set in the subclasses __init__()-method."
        assert hasattr(
            self, "path"), "The 'path' attribute has to be set in the subclasses __init__()-method."
        assert hasattr(
            self, "id_to_file"), "The 'id_to_file attribute' has to be set in the subclasses __init__()-method."
        assert hasattr(
            self, "id_to_target_name"), "The 'id_to_target_name' attribute has to be set in the subclasses __init__()-method."
        assert hasattr(
            self, "categorical_features"), "The 'categorical_features' attribute has to be set in the subclasses __init__()-method."

        self.path_dataset = str(dataset_iml.project_dir) + \
            "/" + self.path + "/" + self.id_to_file[self.id]
        ...
        self.levels_per_feature = np.apply_along_axis(
            lambda x: len(np.unique(x)), axis=0, arr=self.X)
        self.categorical_features_idx = [self.feature_name_to_index(
            feature_name) for feature_name in self.categorical_features]
        self.continuous_features = list(
            set(self.feature_names)-set(self.categorical_features))
        self.continuous_features_idx = [self.feature_name_to_index(
            feature_name) for feature_name in self.continuous_features]
    ...
    def load(self,
             ...
             # y_test_desired: Optional[int]= None,
        ...
        if not split:
            ...
            if n_train:
                if (n_train <= self.max_n_train) and (n_train < self.num_samples):
                    ...
                    n_train = min(self.max_n_train, int(
                        self.num_samples * 0.66))
            else:
            ...
            if to_torch_tensor:
                ...
                y_test = torch.tensor(
                    y_test, device=device).type(torch.LongTensor)
    ...
    def feature_name_to_index(self,
        ...
        try:
            ...
            raise Exception(
                "The specified feature name does not occur in the dataset.")
...
class OpenMLData(dataset_iml):
    def __init__(self,
        ...
        def fetch_openml_dataset(did):
            ...
            if X.shape[0] > 1024 and not avoid_pruning:
                X, _, y, _ = train_test_split(
                    X, y, train_size=1024, stratify=y, random_state=seed)

            # Find indices of categorical features
            categorical_features_idx = [i for i, is_categorical in enumerate(
                categorical_indicator) if is_categorical]
        ...
        self.df['target'] = y
        self.categorical_features = [feature_names[i]
        ...
        self.continuous_features = list(
            set(self.feature_names[:-1]) - set(self.categorical_features))
        self.continuous_features_idx = [i for i, feature_name in enumerate(
            self.feature_names[:-1]) if feature_name in self.continuous_features]
