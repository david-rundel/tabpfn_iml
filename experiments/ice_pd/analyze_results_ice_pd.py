...
def get_latest_csv_file(directory: str,
    ...
    if criterion is not None:
        filtered_files = [file for file in files if file.startswith(
            prefix) and criterion not in file]
    else:
...
df_pdp = get_latest_csv_file(
    'experiments/ice_pd/results', 'benchmark_results_pdp_', 'openml')
...
df_ale = get_latest_csv_file(
    'experiments/ice_pd/results', 'benchmark_results_ale_', 'openml')
...
df_pdp_openml = get_latest_csv_file(
    'experiments/ice_pd/results', 'benchmark_results_pdp_openml_')

barplot_pdp_openml = plot_barplot(df=df_pdp_openml,
                                  x="Number of unique Feature Values",
                                  y="Runtime in Seconds",
                                  hue="Implementation",
                                  hue_order=["tabpfn_iml PDP",
                                             "scikit-learn PDP"],
                                  title="PDP Runtime Comparison",
                                  palette={"tabpfn_iml PDP": cmap_inverted.colors[100], "scikit-learn PDP": cmap_inverted.colors[230]})

barplot_pdp_openml.savefig(
    'experiments/ice_pd/pdp_runtime_comparison_openml.pdf')


df_ale_openml = get_latest_csv_file(
    'experiments/ice_pd/results', 'benchmark_results_ale_openml_')
...
barplot_ale_openml.savefig(
    'experiments/ice_pd/ale_runtime_comparison_openml.pdf')
