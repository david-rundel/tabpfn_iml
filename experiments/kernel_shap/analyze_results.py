...
if rectify_indices:
    for openml_id, temp_exp_results in exp_results.items():
        temp_error_exact_marg_runs = temp_exp_results["error_exact_marginalization_per_run"]
        temp_error_appr_marg_runs = temp_exp_results["error_appr_marginalization_per_run"]
        temp_max_L = temp_exp_results["experiment_hyperparameters"]["max_L"]
        temp_runs = temp_exp_results["experiment_hyperparameters"]["runs"]

        appr_cols = list(range(1, temp_max_L+1))
        exact_col = [-1]
        temp_exp_results["error_exact_marginalization_mean"] = pd.DataFrame(np.concatenate([temp_error_exact_marg_runs[i] for i in range(
            temp_runs)], axis=1).mean(axis=-1), columns=exact_col, index=temp_error_exact_marg_runs[0].index)
        temp_exp_results["error_appr_marginalization_mean"] = pd.DataFrame(np.stack([temp_error_appr_marg_runs[i] for i in range(
            temp_runs)]).mean(axis=0), columns=appr_cols, index=temp_error_appr_marg_runs[0].index)
        temp_exp_results["error_exact_marginalization_std"] = pd.DataFrame(np.concatenate([temp_error_exact_marg_runs[i] for i in range(
            temp_runs)], axis=1).std(axis=-1), columns=exact_col, index=temp_error_exact_marg_runs[0].index)
        temp_exp_results["error_appr_marginalization_std"] = pd.DataFrame(np.stack([temp_error_appr_marg_runs[i] for i in range(
            temp_runs)]).std(axis=0), columns=appr_cols, index=temp_error_appr_marg_runs[0].index)
...
plot_error_per_ML(exp_results, True)
...
plot_error_per_tc(stacked_results, True)
